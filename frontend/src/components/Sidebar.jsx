...
import { LogOut, ChevronLeft, ChevronRight, Headphones } from 'lucide-react'
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 py-4 px-3 space-y-1 overflow-hidden">
          {navConfig.items.map(({ to, label, icon, color }, i) => {
        ...
        <div className={`px-3 py-3 border-t ${theme === 'light' ? 'border-navy-100' : 'border-white/[0.06]'}`}>
          {!collapsed && user && (
            <div className="px-3 py-2 mb-2">
              ...
              <p className={`text-xs truncate ${theme === 'light' ? 'text-navy-400' : 'text-white/30'}`}>{user.role || 'Agent'}</p>
            </div>
          )}
          <button
